...
const DropdownUser = () => {
  ...
  const submitLogOut = async () => {
    queryClient.cancelQueries(); // 모든 실행 쿼리 취소
    return logout('/auth/logout').then(
      async (res: any) => {
        const result = await expireAll();
        if (result) {
          resetLoggedInUser();
          resetUsableMenus();
          nav('/auth/login');
        ...
        >
          <div className="flex items-center justify-end gap-1">
            ...
            <div className="flex items-center gap-1">
              <span className="hidden sm:inline">{`(${loggedInUser?.companyName})`}</span>
              {/* <img
