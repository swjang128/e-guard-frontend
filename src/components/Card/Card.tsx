interface CardProps {
  ...
  size?: 'sm' | 'md' | 'lg' | 'xs' | 'ml'; // Card의 size (풀스크린 (1280px이상) 기준) - 'sm', 'md', 'lg' 사용가능, 옵션을 넣지 않았을시 기본값은 'lg' // xs, ml 추가
...
const Card: React.FC<CardProps> = ({
  ...
  switch (size) {
    ...
    case 'lg':
      ...
      break;
    case 'xs':
      genratedSizeClass = 'xl:col-span-2'; // 1/6 size
      break;
    case 'ml':
      genratedSizeClass = 'xl:col-span-10'; // 5/6 size
      break;
    default:
      throw new Error(
        "'size'가 올바르지 않습니다. : 'size'는 'sm','md','lg','xs','ml' 만 선택할 수 있습니다."
      );
  }
